...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const authText = document.querySelector(".auth-text");
    const sideMenu = document.querySelector(".side-menu");
    const messagesContainer = document.getElementById("messages-container"); // Messages container
    ...
    const hideMessages = () => {
        if (messagesContainer) {
            const timeout = parseInt(messagesContainer.getAttribute("data-timeout")) || 3000; // Default to 3 seconds
            setTimeout(() => {
                messagesContainer.style.transition = "opacity 0.5s ease";
                messagesContainer.style.opacity = 0; // Fade out
                setTimeout(() => {
                    messagesContainer.style.display = "none"; // Hide completely after fading
